...
export const addToGuestCart = async (productVariantId, quantity = 1) => {
    try {
        ...
        const response = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL || (process.env.NODE_ENV === "development" ? "http://localhost:4002/api" : "https://bluebellfnb.com/api")}/public/products/variants/${productVariantId}`,
            {
                credentials: "include",
            }
        );
        ...
        const mergePromises = guestItemsToMerge.map(async (guestItem) => {
            try {
                ...
                const addResponse = await fetch(
                    `${process.env.NEXT_PUBLIC_API_URL || (process.env.NODE_ENV === "development" ? "http://localhost:4002/api" : "https://bluebellfnb.com/api")}/cart/add`,
                    {
                        method: "POST",
                        headers: {
