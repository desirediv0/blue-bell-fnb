...
export const ProductCard = ({ product, viewMode = "grid" }) => {
  ...
  return (
    <div
      className={`group relative bg-white rounded overflow-hidden flex border border-gray-100 transition-all duration-300 hover:shadow-[0_6px_24px_rgba(0,0,0,0.09)] hover:-translate-y-0.5 ${
        isList ? "flex-row w-full min-h-[140px] sm:min-h-[200px]" : "flex-col h-full"
      }`}
    ...
    >
      ...
      <Link
        ...
        className={`block relative overflow-hidden bg-gray-50 flex-shrink-0 ${isList ? "w-28 sm:w-[180px] md:w-[220px]" : ""}`}
      ...
      {/* ── Info area ── */}
      <div className={`flex flex-col flex-grow p-3 gap-2 ${isList ? "sm:p-3 md:p-4 lg:p-5 sm:gap-3" : ""}`}>

        {/* Name */}
        <Link href={`/products/${product.slug}`} className="flex-none">
          <h3 className={`font-semibold text-gray-800 leading-snug hover:text-primary transition-colors ${
            isList ? "text-base md:text-lg line-clamp-1 sm:line-clamp-2" : "text-[13px] line-clamp-2"
          }`}>
            {product.name}
          </h3>
        </Link>
        ...
        {isList && product.description && (
          <div className="flex-1 overflow-hidden sm:pr-4">
            <p className="text-gray-500 text-xs md:text-sm leading-relaxed hidden sm:[display:-webkit-box] line-clamp-2 xl:line-clamp-3 max-w-2xl mt-1">
        ...
        {/* Price + Add to Cart */}
        <div className={`mt-auto pt-2 border-t border-gray-50 flex items-end justify-between gap-2 px-2 ${
          isList ? "sm:pt-4" : ""
        }`}>
          ...
          <div className="leading-tight min-w-0">
            {showPrice ? (
              <>
                <p className={`font-bold ${isList ? "text-base sm:text-lg md:text-xl whitespace-nowrap" : "text-sm truncate"} ${showFlashSaleBadge ? "text-primary" : "text-gray-900"}`}>
          ...
          <button
            ...
            className={`flex-shrink-0 flex items-center justify-center gap-1.5 transition-all duration-150 active:scale-95 disabled:opacity-50
              ${addedToCart
                ? "bg-green-500 text-white"
                : "bg-primary text-white hover:bg-primary/90"
              }
              ${isList ? "px-4 py-2.5 rounded-xl text-sm" : "px-2.5 py-1.5 rounded-lg text-[11px] font-bold"} 
            `}
          >
            {isAddingToCart ? (
              <Loader2 className={`${isList ? "h-4 w-4" : "h-3.5 w-3.5"} animate-spin`} />
            ) : addedToCart ? (
              <svg className={isList ? "h-4 w-4" : "h-3.5 w-3.5"} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={isList ? 3 : 2.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
              </svg>
            ) : (
              <>
                <ShoppingBag className={isList ? "h-4 w-4" : "h-3.5 w-3.5"} />
